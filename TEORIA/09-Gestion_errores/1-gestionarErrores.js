...
/* siempre pueden surgir errores, lo mejor que se puede hacer es tratar de
gestionar siempre los errores, haciendo algo parecido a lo visto con las
promesas
...
console.log(myFunction1("hola"));       // NaN -> no queremos esto
/* queremos que nos lance un error cuando ese tipo de variable que le enviamos
(un string en este caso) no se pueda evaluar */
...
try {
    ...
    console.log(myFunction2(cadena));                       // genera un error
    // saltamos al catch()
    ...
    /* la variable 'e' guarda el mensaje que se ha escrito en:
        throw new Error("mensaje") */

    /* se usa console.error() porque aunque aquí no se vea, a la hora de
    gestionarcerrores y mostrarlos, así se ven mejor, de otra manera diferente
    a un simple console.log */
}
finally {
...
- SyntaxError -> cada vez que hay un fallo de sintaxis (escribir conts en vez
de const o así)
- TypeError -> ej.: pasarle a una función un número cuando espera un booleano
- RangeError -> que estamos intentando acceder a una posición que está pasando
el index de un array por ejemplo
- ReferenceError -> tratar de llamar a una variable/función/... que no existe o
