...
let name = "June";
despedir(name)
console.log(name); // June -> no se ve afectado por el cambio dentro de la función
...
// en los objetos sí se modifica el valor del objeto original al modificar su valor dentro de la función
function saludarPersona(objeto){
    objeto.apellido = "Villar"
    console.log(`Hola ${objeto.nombre} ${objeto.apellido}`); // Hola Juan Villar
}

//saludarPersona(); // da error -> porque intenta acceder a las propiedades del objeto y no le hemos dicho cuál
...
imprimir(1,2,9,"hola",{id:9}); // se imprimirán todos

function suma(...nums){
    console.log(...nums); // 1 2 3 4
    console.log(nums[2]); // 3
    console.log(nums.reduce((a,b) => a + b)); // 10 -> hace la suma
